...
export FQDN="${RENKU_RELEASE}.dev.azure.renku.ch"
...
if test -n "$GITLAB_TOKEN" ; then
  gitlab_app=$(curl -s -X POST ${GITLAB_URL}/api/v4/applications \
                        ...
                        --data "redirect_uri=https://${RENKU_RELEASE}.dev.azure.renku.ch/auth/realms/Renku/broker/dev-renku/endpoint https://${RENKU_RELEASE}.dev.azure.renku.ch/api/auth/gitlab/token https://${RENKU_RELEASE}.dev.azure.renku.ch/api/auth/callback" \
